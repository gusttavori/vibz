...
import { FaMagic, FaBullhorn, FaMoneyBillWave, FaArrowRight, FaSearch, FaTimes } from 'react-icons/fa'; 
            ...
            if (searchTerm.length >= 1) { 
                try {
                    ...
                    if (response.ok) {
                        ...
                        const titleMatches = data.filter(event => 
            ...
            <div className="search-bar-container">
                <div className="search-outer-border-wrapper" ref={searchWrapperRef}>
                    ...
                    <div className="input-wrapper-relative" style={{ flexGrow: 1, position: 'relative', height: '100%' }}>
                        <input 
                            type="text" 
                            placeholder="Pesquise seu evento predileto" 
                            value={searchTerm} 
                            ...
                            {suggestions.map((event) => (
                                <div key={event._id || event.id} className="suggestion-item" onClick={() => handleSuggestionClick(event._id || event.id)}>
                                    ...
                                    <div className="suggestion-info">
                                        ...
                                        <span className="suggestion-date">{new Date(event.date).toLocaleDateString('pt-BR')}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
