...
const DashboardContent = () => {
    ...
    const financialMetrics = useMemo(() => {
        ...
        return { totalRevenue, totalTicketsSold, activeEventsCount };
    }, [myEvents]);
    ...
    const getStatusLabel = (s) => ({ approved: 'Aprovado', pending: 'Análise', rejected: 'Rejeitado' }[s?.toLowerCase()] || s);
        ...
        <div className="dashboard-container">
            ...
            <main className="dashboard-content">
                ...
                <div className="wallet-section">
                    <div className={`wallet-card ${isStripeReady ? 'success' : 'warning'}`}>
                        ...
                        <div className="wallet-info">
                            ...
                            <p>{isStripeReady ? 'Você está pronto para receber suas vendas.' : 'Conecte sua conta Stripe para vender ingressos.'}</p>
                        </div>
                ...
                <div className="section-header">
                    <h2><FaCalendarAlt /> Gerenciar Eventos</h2>
                        ...
                        myEvents.map((event) => (
                            <div key={event.id || event._id} className="event-card-dash">
                                    ...
                                    <div className="event-meta">
                                        <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                            ...
                                            {event.highlightStatus === 'paid' && (
                                                <FaStar color="#F59E0B" title="Destaque Ativo" />
                                            )}
                                        </div>
                                        ...
                                        <div className="status-row" style={{display:'flex', gap:'8px', marginTop:'5px', alignItems:'center'}}>
                                            ...
                                            {event.highlightStatus === 'pending' && (
                                                <span className="badge highlight-pending">⏳ Destaque em análise</span>
                                            )}
                                            ...
                                            {event.highlightStatus === 'paid' && (
                                                <span className="badge highlight-active">
                                                    🌟 Destaque Ativo
                                                </span>
                                            )}
                                        </div>
                                ...
                                <div className="event-actions">
                                    ...
                                    <button className="btn-icon" onClick={() => router.push(`/eventos/editar/${event.id || event._id}`)}>
                                        <FaEdit /> Editar
                                    </button>
                                </div>
                            </div>
