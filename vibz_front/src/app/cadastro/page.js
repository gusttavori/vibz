  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/auth/register`, {
        ...
        credentials: 'include', // <-- Essencial para o navegador receber e salvar o Cookie HttpOnly
      ...
      if (response.ok) {
        ...
        // Mantém apenas os dados básicos para o Header reconhecer o usuário
        if(data.user) {
            localStorage.setItem('userId', data.user.id || data.user._id);
            localStorage.setItem('userName', data.user.name);
        }
        
        // Como a sessão já está nos Cookies, podemos jogar direto para a Home
        setTimeout(() => {
