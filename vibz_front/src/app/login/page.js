      ...
      if (code) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            // 🛡️ MUDANÇA CRUCIAL AQUI: Verifica se o JSON realmente contém os dados do usuário!
            if (data && data.user) {
                // Usuário já está logado de verdade.
                window.location.replace('/');
            } else {
                // O backend retornou 200 OK, mas avisando que NÃO está logado ({ logado: false })
                setIsCheckingAuth(false);
            }
          } else {
            setIsCheckingAuth(false);
