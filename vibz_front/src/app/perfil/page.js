...
export default function UserProfile() {
    ...
    const [mainTab, setMainTab] = useState('ingressos'); 
    const [ticketTab, setTicketTab] = useState('valid'); // 'valid' = Próximos, 'history' = Histórico
    const [selectedTicket, setSelectedTicket] = useState(null);

    useEffect(() => {
        const fetchAllData = async () => {
            try {
                // 1. Busca Usuário e Favoritos com Segurança (Cookie)
                const profileRes = await fetch(`${API_BASE_URL}/users/me`, {
                    credentials: 'include', // <-- MÁGICA AQUI
                    cache: 'no-store' 
                });
                ...
                // 2. Busca Ingressos com Segurança (Cookie)
                try {
    ...
    // --- FUNÇÕES DE FAVORITOS (VIA COOKIE) ---
    ...
    // --- NOVA LÓGICA DE DATAS E FILTRAGEM ---
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const getTicketDate = (ticket) => {
        const dateStr = ticket.ticketType?.activityDate || ticket.event?.eventDate || ticket.event?.date || ticket.event?.createdAt;
        if (!dateStr) return new Date(); 
        try {
            const cleanDate = dateStr.toString().includes('T') ? dateStr.split('T')[0] : dateStr;
            const [year, month, day] = cleanDate.split('-');
    ...
    const activeTickets = tickets.filter(t => t.event !== null);
    const upcomingTickets = activeTickets.filter(t => getTicketDate(t) >= today);
    const historyTickets = activeTickets.filter(t => getTicketDate(t) < today);
    const currentTicketsList = ticketTab === 'valid' ? upcomingTickets : historyTickets;
                        ...
                        <div className="tickets-grid">
                            {currentTicketsList.length === 0 ? (
                                ...
                                currentTicketsList.map((ticket) => {
                                    const dateToShow = ticket.ticketType?.activityDate || ticket.event?.date || ticket.event?.eventDate;
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="wallet-ticket-info">
                                                ...
                                                <h3>{formatText(ticket.event?.title)}</h3>
                        ...
                        <div className="modal-body">
                            <div className="qr-container">
                                {selectedTicket.qrCodeData ? (
                                    <img 
                                        src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${selectedTicket.qrCodeData}&color=4C01B5`} 
                                        alt="QR Code do seu ingresso. Apresente na entrada." 
                                        className="qr-image"
                                    />
                                ) : (
                                    <div className="qr-loading" role="status">Código não encontrado</div>
                                )}
                            ...
                            <div className="ticket-info-block">
                                ...
                                <div className="info-row">
                                    <span>Data</span>
                                    <strong>{formatDate(selectedTicket.ticketType?.activityDate || selectedTicket.event?.date || selectedTicket.event?.eventDate)}</strong>
                                </div>
                                ...
                                <div className="status-row">
                                    <span 
                                        ...
                                        role="status"
                                        aria-label={`Status do ingresso: ${selectedTicket.status === 'valid' ? 'Pronto para uso' : 'Já utilizado'}`}
                                    >
                                        {selectedTicket.status === 'valid' ? 'PRONTO PARA USO' : 'JÁ UTILIZADO'}
                                    </span>
                                </div>
