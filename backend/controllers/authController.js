...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        // ENVIA O TOKEN VIA COOKIE SEGURO
        res.cookie('vibz_token', token, cookieOptions);
        
        // RETORNA OS DADOS SEM O TOKEN EXPOSTO NO JSON
        res.status(201).json({ msg: 'Sucesso!', user: { id: user.id, _id: user.id, name: user.name, email: user.email } });
    } catch (err) {
...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        // RETORNA OS DADOS SEM O TOKEN EXPOSTO NO JSON
        res.json({ msg: 'Login OK!', user: { id: user.id, _id: user.id, name: user.name, email: user.email, isAdmin: user.isAdmin } });
    } catch (err) {
...
const googleLogin = async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            const token = generateToken(user.id);
            res.cookie('vibz_token', token, cookieOptions);
            return res.json({ msg: "Login Google OK!", user: { id: user.id, _id: user.id, name: user.name, email: user.email } });
        } else {
            ...
            const token = generateToken(user.id);
            res.cookie('vibz_token', token, cookieOptions);
            return res.status(201).json({ msg: "Cadastro Google OK!", user: { id: user.id, _id: user.id, name: user.name, email: user.email } });
        }
    } catch (err) {
...
module.exports = {
    registerUser, loginUser, googleLogin, logoutUser, forgotPassword, resetPassword, validateResetCode, getMe
};